...
function cleanDashboardData() {
  dashboardData.leaderWorkloadSummary = dashboardData.leaderWorkloadSummary.map((leader) => ({
    ...leader,
    service_lines: toArray(leader.service_lines || leader.serviceLine || leader["Service Lines"]).map(normalizeServiceLine).filter(Boolean)
  }));

  dashboardData.leaderDrilldown = dashboardData.leaderDrilldown.map((leader) => ({
    ...leader,
    service_lines: toArray(leader.service_lines || leader.serviceLine || leader["Service Lines"]).map(normalizeServiceLine).filter(Boolean),
    assigned_opportunities: toArray(leader.assigned_opportunities)
  }));

  dashboardData.newOpportunities = dashboardData.newOpportunities.map((opp) => ({
    ...opp,
    service_line: normalizeServiceLine(opp.service_line || opp.serviceLine || opp["Service Line"])
  ...
  const nodes = toArray(dashboardData.networkNodesEdges.nodes).map((node) => {
    if ((node.type || node.group) === "service_line") {
      return { ...node, id: normalizeServiceLine(node.id || node.label), label: normalizeServiceLine(node.label || node.id), type: "service_line" };
    }
    return node;
  }).filter((node) => {
...
function updateDataStatus() {
  ...
  if (!status) return;
  status.textContent = loadedAny ? "Model outputs loaded" : "Using empty data structure";
  status.className = loadedAny ? "status-pill success" : "status-pill error";
}
...
function renderHome() {
  ...
  const cards = [
    {
      label: "Total Leaders",
      value: metrics.total_leaders ?? leaders.length,
    ...
    },
    {
      ...
      value: metrics.total_new_opportunities ?? dashboardData.newOpportunities.length,
      note: "Opportunities to absorb"
    },
    {
      label: "Capacity Watch",
      value: metrics.capacity_watch_count ?? overCapacity + nearCapacity,
